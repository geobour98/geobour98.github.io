...
So, now the above shellcode is declared in the the crypter program [`aesencrypt.c`](https://github.com/geobour98/slae64/blob/main/7_Custom_crypter/aesencrypt.c). It is very important to note that since we are using 32 byte blocks, then the size of the shellcode must be padded until we reach 32 (or another number divisinle by 32) bytes. Fortunately, the size of the original shellcode is `32`, so we don't need to add more bytes. 
...
So, now the above shellcode is declared in the decrypter program [`aesdecrypt.c`](https://github.com/geobour98/slae64/blob/main/7_Custom_crypter/aesdecrypt.c).
