...
This is the blog post for the 6{{< rawhtml >}}<sup>th</sup>{{< /rawhtml >}} Assignment of the SLAE64 course, which is offered by [PentesterAcademy](https://www.pentesteracademy.com/course?id=7). The course focuses on teaching the basics of 64-bit Assembly language for the Intel Architecture (IA-x86_64) family of processors on the Linux platform.

The purpose of this assignment is to take 3 shellcodes from [Shell-Storm](https://shell-storm.org/shellcode/index.html) from the **Intel x86-64** category and create polymorphic versions of them in order to beat pattern matching. Pattern matching means that a program (AV or IDS) has a database with **signatures**. A signature is bytes suite identifying a program. More information can be found at:  [Polymorphic Shellcode Engine, by Phrack](http://phrack.org/issues/61/9.html). So, with polymorphism we preserve the functionality of the shellcode, by using equivalent instructions or garbage instructions that don't change the functionality at all, like `NOP`. 
...
| -- | -- | -- | -- |
| [:link:execve](#execve) | [Shell-Storm execve](https://shell-storm.org/shellcode/files/shellcode-76.html) | 41 | 35 |
| [:link:tcpbindshell](#tcpbindshell) | [Shell-Storm tcpbindshell](https://shell-storm.org/shellcode/files/shellcode-858.html) | 150 | 124 |
| [:link:tcpreverseshell](#tcpreverseshell) | [Shell-Storm tcpreverseshell](https://shell-storm.org/shellcode/files/shellcode-857.html) | 118 | 109 |  

My code can be found in my Github: [geobour98's Github](https://github.com/geobour98/slae64).
...
It should be noted that the author had declared the size of the shellcode as `33`, but after compiling and running the shellcode tester `C` program (`shellcode-original.c`) the size was found to be `41` bytes. So, we stick to the 2{{< rawhtml >}}<sup>nd</sup>{{< /rawhtml >}} value as the size.
...
1{{< rawhtml >}}<sup>st</sup>{{< /rawhtml >}} window:
...
2{{< rawhtml >}}<sup>nd</sup>{{< /rawhtml >}} window:
...
1{{< rawhtml >}}<sup>st</sup>{{< /rawhtml >}} window again:
