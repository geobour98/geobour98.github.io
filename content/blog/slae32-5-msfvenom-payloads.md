...
| -- | -- | -- |
| [:link:linux/x86/chmod](#chmod) | Runs chmod on specified file with specified mode | 36 |
| [:link:linux/x86/exec](#exec) | Execute an arbitrary coommand or just a /bin/sh shell | 20 |
| [:link:linux/x86/read_file](#read_file) | Read up to 4096 bytes from the local file system and write it back out to the specified file descriptor | 62 |
...
{{< rawhtml >}}
  <img src="../../../blog/slae32/exec_shellcode.png" alt="Exec shellcode photo" style="width:700px;height:270px;">
{{< /rawhtml >}}
...
Shellcode Length:  4
root:{{< rawhtml >}}<zero-width space>{{< /rawhtml >}}x:0:0:root:/root:/bin/bash
daemon:{{< rawhtml >}}<zero-width space>{{< /rawhtml >}}x:1:1:daemon:/usr/sbin:/usr/sbin/nologin
bin:{{< rawhtml >}}<zero-width space>{{< /rawhtml >}}x:2:2:bin:/bin:/usr/sbin/nologin
<snip>
geobour98:{{< rawhtml >}}<zero-width space>{{< /rawhtml >}}x:1000:1000:George Bourlakis,,,:/home/geobour98:/bin/bash
vboxadd:{{< rawhtml >}}<zero-width space>{{< /rawhtml >}}x:999:1::/var/run/vboxadd:/bin/false
sshd:{{< rawhtml >}}<zero-width space>{{< /rawhtml >}}x:121:65534::/var/run/sshd:/usr/sbin/nologin
```
